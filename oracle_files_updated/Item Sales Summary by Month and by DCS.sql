select a.sbs_no, a.store_code,extract(year from a.invc_post_date) YEAR,extract(month from a.invc_post_date) "MONTH",d.dcs_code DCS_CODE,to_char(i.upc) upc,i.description1,
...
and =-=s.sbs_no=-= and =-=a.store_no=-= and =-=trunc(a.invc_post_date)=-=
group by a.sbs_no,a.store_no,a.store_code,extract(year from a.invc_post_date) ,extract(month from a.invc_post_date),d.dcs_code,i.upc,i.description1
order by a.sbs_no,a.store_no,a.store_code,extract(year from a.invc_post_date) ,extract(month from a.invc_post_date),d.dcs_code,i.upc,i.description1