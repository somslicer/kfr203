...
sum(nvl(sal.qty,0)) Sold_qty,round(sum(nvl(sal.val,0)),2) Net_Sold_Value_W_Tax, i.last_rcvd_date Last_received_date,
sum(iq.qty) On_Hand,round(sum(iq.qty*nvl(ip.price,0)),2) On_Hand_Value,
dense_rank() over(partition by s.sbs_no, st.store_no order by sum(nvl(sal.qty,0)) asc) Rank
