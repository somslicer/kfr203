...
ROUND( CASE WHEN LC=0 THEN 0 ELSE (Sal.LOPT-Sal.LC)/(sal.LC) END,2) "LAST GMROI",
	...
	from	   
	(select s.sbs_no, s.sbs_name, b.dcs_code, sum(case when =-=cast(a.Invc_Post_Date as date)=-= then (CASE WHEN b.item_type = 2 THEN b.qty * -1 ELSE b.qty END)  else 0 end) LSQ,
