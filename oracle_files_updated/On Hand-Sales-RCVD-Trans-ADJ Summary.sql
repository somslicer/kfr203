...
JOIN (
  ...
  FROM (
  SELECT
      d2.sbs_no, d2.store_no, d2.doc_type, d2.invc_post_date,
      d2.item_sid, d2.open_Qty, d2.qty, d2.close_Qty,
      rank() OVER (PARTITION BY d2.sbs_no, d2.store_no, d2.item_sid ORDER BY d2.invc_post_date) AS doc_open_rank,
      rank() OVER (PARTITION BY d2.sbs_no, d2.store_no, d2.item_sid ORDER BY d2.invc_post_date DESC) AS doc_close_rank
  FROM(
    SELECT 
      d.sbs_no,
      d.store_no,
      ...
      d.invc_post_date,
      d.item_sid,
      SUM (d.qty) OVER (PARTITION BY d.sbs_no, d.store_no, d.item_sid ORDER BY d.invc_post_date DESC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS open_Qty,
      d.qty,
      CASE WHEN DOC_TYPE ='INVN' THEN d.qty
          ELSE (SUM (d.qty) OVER (PARTITION BY d.sbs_no, d.store_no, d.item_sid ORDER BY d.invc_post_date DESC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW))  - d.qty 
          END AS close_Qty
    FROM (
      SELECT 
          s.sbs_no,
          st.store_no,
          'INVN' AS doc_type,
          (SYSTIMESTAMP) AS invc_post_date,
          i.sid AS item_sid,
          NVL(iq.qty, 0) AS qty
      FROM rps.invn_sbs_item i
      JOIN rps.invn_sbs_item_qty iq ON i.sid =iq.invn_sbs_item_sid AND i.sbs_sid=iq.sbs_sid
      ...
      SELECT 
          ...
          a.store_no,
          'SALE' AS doc_type,
          (a.invc_post_date) AS invc_post_date,
