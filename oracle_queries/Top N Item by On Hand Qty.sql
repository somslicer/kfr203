...
i.alu,to_char(i.upc) UPC,i.description1,i.description2,v.vend_code "VEND_CODE", v.vend_name "VEND_NAME", i.last_sold_date,
sum(nvl(iq.qty,0)) On_Hand,round(sum(nvl(iq.qty,0)*nvl(ip.price,0)),2) On_Hand_Value,
sum(nvl(sal.qty,0)) Sold_qty,round(sum(nvl(sal.val,0)),2) Net_Sold_Value_W_Tax,
dense_rank() over(partition by s.sbs_no, st.store_no order by sum(nvl(iq.qty,0)) desc) Rank
...
sum(decode(b.item_type,2,b.qty*-1,b.qty)*decode(a.use_vat,1,b.price, b.price+b.tax_amt+b.tax2_amt)) val
...
i.alu,i.upc,i.description1,i.description2,v.vend_code,v.vend_name, i.last_sold_date
) aa
where 1=1 and =-=ranks=-=